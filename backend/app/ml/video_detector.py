...
def normalize_video(input_path: str) -> str:
    ...
    Gracefully falls back to the original video if FFmpeg is not installed.
    """
    ...
    try:
        ...
        if result.returncode != 0:
            logger.warning("[VIDEO] FFmpeg normalization failed (code %d): %s", result.returncode, result.stderr.decode(errors="replace")[:200])
            return input_path
        return output_path
    except FileNotFoundError:
        logger.warning("[VIDEO] FFmpeg not found, skipping normalization. Using original video.")
        return input_path
    except Exception as exc:
        logger.warning("[VIDEO] Unexpected error during normalization, using original video: %s", exc)
        return input_path
...
def _extract_frames(video_path: str) -> list[dict]:
    ...
    # Dynamically increase sample rate for short videos (e.g., 3s videos)
    effective_sample_rate = 0.5 if duration_sec <= 5.0 else SAMPLE_EVERY_N_SEC
    frame_interval = max(1, int(fps * effective_sample_rate))
