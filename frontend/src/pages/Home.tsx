...
const Home: React.FC = () => {
  ...
  const [showFloatingDown, setShowFloatingDown] = useState(false);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (scrollY > 50 && scrollY + winHeight < docHeight - 150) {
        setShowFloatingDown(true);
      } else {
        setShowFloatingDown(false);
      }
    };
  ...
  return (
    <div className="w-full bg-[#F5F0E8] text-[#374151] overflow-x-hidden pt-8">
      ...
      {/* Static Scroll to Top Arrow */}
      <div className="w-full flex justify-center pb-12 pt-6 bg-[#F5F0E8]">
        <button 
          onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
          className="p-3 bg-transparent text-gray-500 border-[2px] border-gray-300 rounded-full shadow-[4px_4px_0_#E5E7EB] hover:-translate-y-2 hover:-translate-x-1 hover:shadow-[5px_5px_0_#D1D5DB] transition-all group cursor-pointer"
      ...
      {showFloatingDown && (
        <div className="fixed bottom-8 left-0 right-0 mx-auto w-fit z-50 animate-fadeIn">
          <button 
